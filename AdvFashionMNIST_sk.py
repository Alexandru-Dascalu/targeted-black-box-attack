...
def loadData(): 
    (dataTrain, labelsTrain, dataTest, labelsTest) = MNIST.loadHDF5()
...
class NetCIFAR10(Nets.Net):
    ...
    def train(self, genTrain, genTest, pathLoad=None, pathSave=None):
        with self._graph.as_default(): 
            ...
            self._lrDecay1 = tf.compat.v1.assign(self._lr, self._lr * 0.2)
            ...
            self._optimizerP = tf.compat.v1.train.AdamOptimizer(self._lr2, epsilon=1e-8).minimize(self._lossPredictor, var_list=self._varsP)
...
#             self.sample(genTest)
            ...
            while globalStep < self._HParam['TotalSteps']: 
                ...
                if (globalStep % 6000 == 0 or globalStep % 9000 == 0) and globalStep < 10000: 
                    self._sess.run(self._lrDecay1)
...
if __name__ == '__main__':
    ...
    enemy = skModels.NaiveBayes(dataTrain, labelsTrain)
    #enemy = skModels.RandomForest(dataTrain, labelsTrain)
    ...
    net.train(batchTrain, batchTest, pathSave='./AttackFashionMNIST/netcifar10.ckpt')
    #net.sample(batchTest, './AttackMNIST/netcifar10.ckpt-1500')
