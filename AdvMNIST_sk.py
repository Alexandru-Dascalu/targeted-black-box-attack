...
def loadData(): 
    (dataTrain, labelsTrain, dataTest, labelsTest) = MNIST.lodHDF5()
    return dataTrain.reshape([-1, 784]), labelsTrain.reshape([-1]), dataTest.reshape([-1, 784]), labelsTest.reshape([-1]) 
...
class NetCIFAR10(Nets.Net):
    ...
    def train(self, genTrain, genTest, pathLoad=None, pathSave=None):
        with self._graph.as_default(): 
            ...
            self._lrDecay2 = tf.compat.v1.assign(self._lr2, self._lr2 * 0.2)
            ...
            if pathLoad is not None:
                ...
                for idx in range(300): 
                    ...
                    adversary = data
                    label = np.array(self._enemy.predict(data.reshape([-1, 784])))
                    loss, accu, _ = \
                ...
                for idx in range(50): 
                    ...
                    adversary = data
                    label = np.array(self._enemy.predict(adversary.reshape([-1, 784])))
                    loss, accu, _ = \
                        self._sess.run([self._lossPredictor,
                                        self._accuracy, self._optimizerP],
                                       feed_dict={self._images: adversary,
...
#             self.sample(genTest)
            ...
            while globalStep < self._HParam['TotalSteps']: 
                ...
                for _ in range(self._HParam['NumGenerator']): 
                    data, label, target = next(genTrain)
                    refs = np.array(self._enemy.predict(data.reshape([-1, 784])))
                    ...
                    loss, adversary, globalStep, _ = \
                        self._sess.run([self._lossGenerator,
                                       ...
                                       feed_dict={self._images: data,
                                                  self._labels: refs,
                                                  self._targets: target})
                    results = np.array(self._enemy.predict(adversary.reshape([-1, 784])))
                ...
                if (globalStep % 5700 == 0 or globalStep % 7200 == 0) and globalStep < 10000: 
                    self._sess.run(self._lrDecay1)
...
if __name__ == '__main__':
    import pickle
    # enemy = MNIST.NetCIFAR10([28, 28, 1], 2)
    # enemy.load('./ClassifyMNIST/netcifar10.ckpt-16200')
    (dataTrain, labelsTrain, dataTest, labelsTest) = loadData()
    #enemy = skModels.NaiveBayes(dataTrain, labelsTrain) # 56%
    enemy = skModels.LogisticRegression(dataTrain, labelsTrain) # Accu:  0.9980999999999998 ; FullRate:  0.9995999999999999
    #enemy = skModels.RandomForest(dataTrain, labelsTrain) # 30% Accu:  0.2803 ; FullRate:  0.8209
    #enemy = skModels.DecisionTree(dataTrain, labelsTrain) # Accu:  0.48500000000000015 ; FullRate:  0.8994
    #enemy = pickle.load(open('./AttackMNIST/Enemy_RandomForest.pkl', 'rb'))
    #enemy = skModels.SVM(dataTrain, labelsTrain)
    #pickle.dump(enemy, open('./AttackMNIST/Enemy_RandomForest.pkl', 'wb'))
    ...
    batchTrain, batchTest = MNIST.get_adversarial_data_generators(batch_size=HParamCIFAR10['BatchSize'], preproc_size=[28, 28, 1])
    ...
    while True: 
        net.sample(batchTest, './AttackMNIST/netcifar10.ckpt-4200')
